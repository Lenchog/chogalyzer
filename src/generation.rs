...
#[must_use] pub fn generate_threads(layout_raw: [char; 32], corpus: &String, max_iterations: u64, magic_rules: usize, cooling_rate: f64) -> ([char; 32], i64, Vec<String>) {
    ...
    thread::scope(|s| {
      let vec: Vec<_> = (0..THREADS).map(|_| s.spawn(|| generate(layout_raw, corpus, max_iterations, &bars, magic_rules, cooling_rate) )).collect();
...
fn generate(layout_raw: [char; 32], corpus: &String, max_iterations: u64, multibars: &MultiProgress, magic_rules: usize, cooling_rate: f64) -> ([char; 32], i64, Vec<String>) {
...
fn attempt_swap(
    ...
    if new_stats.score > old_stats.score || annealing_func(old_stats.score, new_stats.score, temparature) {
...
fn compare_layouts(layouts: [([char; 32], i64, Vec<String>); THREADS]) -> ([char; 32], i64, Vec<String>) {
...
fn annealing_func(old: i64, new: i64, temparature: f64) -> bool {
    let mut rng = ThreadRng::default();
    let delta: f64 = -old as f64 - -new as f64 ;
